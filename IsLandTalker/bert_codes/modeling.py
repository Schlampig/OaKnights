...
#         attention_probs = attention_probs.to(dtype=torch.float16)
...
class UniLM(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, context_mask=None, output_all_encoded_layers=True):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
...
class BertForUniLMInference(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, context_mask=None, output_idx=None):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, context_mask=context_mask, output_all_encoded_layers=False)
...
class Seq2SeqModel(PreTrainedBertModel):
    def __init__(self, config):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, context_mask=None, output_idx=None, is_train=False):
        squence_out, _ = self.bert(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask, context_mask=context_mask, output_all_encoded_layers=False)
        
        if is_train:
            predictions = self.cls(squence_out)  # [batch, len, vocab_size]
            ...
            output_idx = output_idx.unsqueeze(-1).repeat(1, 1, squence_out.shape[2])
            squence_out = torch.gather(squence_out, dim=1, index=output_idx)
            predictions = self.cls(squence_out)
