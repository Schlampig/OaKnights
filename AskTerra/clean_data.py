...
def parser_all(is_save=True):
    ...
    """
    d_all = dict()
    root_dir = "ArknightsData-master/zh-CN/gamedata/story"
    print("- 解析主线 -")
...
def group_dialogue(lst):
    ...
    :return: list, lst_new where context from same person have been grouped
    """
    lst_new = list()
    tag = None  # 记录当前说话人
    talk = ""  # 记录当前说话人的谈话内容
...
def avg_dialogue(lst, avg_len=32):
    ...
    :return: list, lst_new where contexts have similar length
    """
    lst_new = list()
    talk = ""  # 记录当前列表片段的谈话内容
    for dialogue in lst:
...
def filter_background(s):
    ...
    :return: boolean, True to keep False to discard
    """
    lst_s = s.split(":")
    if len(lst_s) == 2:
...
def example(do_tag=0):
    ...
    # 3. 获取全部对话文件内容
    if do_tag == 2:
        _ = parser_all()  # 注：由于打印内容过多，建议设置断点查看，而不要直接打印
    # 4. 生成训练、验证集
    if do_tag == 3:
        lst_train, lst_dev = split_story(load_path="story_raw.json", save_path="story", operation="group")
        print(lst_train[:15])
        print("-"*15)
        print(lst_dev[:15])
    return None
